...
const Scroller = function(container, options) {
  ...
  var component = (self.__component = tempContainer.querySelector(
    '[data-role=component]'
  ));
  var content = (self.__content = component.querySelector(
    '[data-role=content]'
  ));
  ...
  if (component.clientHeight === 0) {
    self.__setDimensions(
      parseFloat(getComputedStyle(component, 'height'), 10),
      204
    );
  }
...
var members = {
  ...
  __selectItem(selectedItem) {
    ...
    var lastSelectedElem = self.__content.querySelector(
      '.' + selectedItemClass
    );
    if (lastSelectedElem) {
